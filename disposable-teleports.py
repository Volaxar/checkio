# https://py.checkio.org/mission/disposable-teleports/


def build_route(step, route, links):
    route.append(step)

    if len(set(route)) == 8 and step == '1':
        return True

    for x in set().union(*filter({step}.intersection, links)) - {step}:
        links.remove({step, x})

        if build_route(x, route, links):
            return True
...
def checkio(teleports_string):
    route = []
    build_route('1', route, [*map(set, teleports_string.split(","))])
