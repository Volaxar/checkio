...
def checkio(f, g):
    def h(*args, **kwargs):
        ...
        for name, func in funcs.items():
            try:
                funcs[name] = func(*args, **kwargs)
            except:
                funcs[name] = None

        if all(x is not None for x in funcs.values()):
            return funcs['g'], 'same' if funcs['f'] == funcs['g'] else 'different'

        for name, func in funcs.items():
            if func is not None:
                return func, '{}_error'.format(name)
